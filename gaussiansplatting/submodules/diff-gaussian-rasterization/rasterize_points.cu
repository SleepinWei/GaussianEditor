	...
	const bool include_feature)
{
	...
	torch::Tensor out_depth = torch::full({1, H, W}, 1e5, float_opts);
	...
	return std::make_tuple(rendered, out_color, out_language_feature, out_depth,out_alpha, radii, geomBuffer, binningBuffer, imgBuffer);
}
